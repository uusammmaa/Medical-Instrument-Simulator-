...
// Generate signal data function
const generateSignalData = (signal: SignalData, distortion: boolean, time: number): number => {
  let value = 0;
...
// Pure function to render signal to canvas
export const renderSignal = ({
  ctx,
  signal,
  index,
  width,
  ...
  if (signalBuffer && signalBuffer.length > 0) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      const y = centerY + generateSignalData(signal, distortion, time) * amplitude;
  ...
  const currentValue = signalBuffer && signalBuffer.length > 0 
    ...
    : generateSignalData(signal, distortion, currentTime);
